...
func init_inotify() {
    // Initializes inotify and epoll file descriptors.
    var err error

    // Creates the maps for translating between names and WD.
    name_by_wd = make(map[int32]string)
    wd_by_name = make(map[string]int32)

    // Initializes inotify_fd and the event size.
    ...
    // Initializes epoll_fd and sets up event watching for inotify_fd.
    ...
    // Starts a goroutine to observe file system events.
    go inotify_observe()
}

func inotify_add_watch(name string) {
    // Adds a watch for the given file or directory.
...
func inotify_rm_watch(name string) {
    // Removes the watch for the given file or directory.
...
func inotify_observe() {
    // Monitors file system events and handles them accordingly.
...
func inotify_observe_collect(buf []byte) map[string]int {
    // Collects file system events to process them later.
...
func inotify_dialog(s map[string]int) bool {
    // Shows a dialog for handling file system events.
    if nil == accel_group {
