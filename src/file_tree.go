...
func file_tree_insert(name string) {
	file_tree_insert_rec(&file_tree_root, name)
}

func file_tree_insert_rec(root *FileTreeNode, name string) {
	...
	if pos == child_name_len {
		...
		file_tree_insert_rec(cur_child, name[child_name_len:])
		return
	}
	...
	file_tree_insert_rec(replacement, name[pos:])
}
...
func file_tree_merge_parent_and_child(child *FileTreeNode) {
	...
	if parent != &file_tree_root {
		...
		if name_is_dir(new_name) {
			...
			file_tree_insert_rec(grand_parent, new_name)
		}
	}
}
