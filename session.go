...
// IgnoreMap is a map of regular expressions used for ignoring files.
type IgnoreMap map[string]*regexp.Regexp

var ignore IgnoreMap

// file_is_saved checks if the file has been saved.
func file_is_saved(file string) bool {
    return strings.Index(file, string(os.PathSeparator)) != -1
}

// name_is_ignored checks if the name is ignored.
...
// get_stack_set_add_file adds the file to the stack set.
...
// get_stack_set returns the set of files contained in the stack + cur_file.
// It also returns the reverse list of files from stack without duplications
// preceded by cur_file.
...
// get_file_info returns the file info for a given file.
...
// session_save saves the session.
func session_save() {
    file_save_current()
...
// session_open_and_read_file opens and reads a file for the session.
...
// session_restore restores the session.
...
// take_reader_from_file takes a reader from a file.
...
// next_string_from_reader gets the next string from a reader.
