// Package main contains the implementation of a file tree using go-gtk.
package main
...
// NewFileTreeNode creates and returns a new FileTreeNode.
...
// name_is_dir returns true if the given name represents a directory, false otherwise.
func name_is_dir(name string) bool {
	return ('/' == name[len(name)-1])
}

// slashed_prefix returns the length of the common slashed prefix of a and b.
...
// file_tree_find_among_children finds a node among the children of the given root node with the given name.
// If the node is found, it returns the node, its position in the list of children (if any) and the previous sibling (if any).
// If the node is not found, it returns the last child node smaller than the given name as well as its previous sibling (if any).
// In case there are no children, it returns nil.
...
// file_tree_insert inserts a new node with the given name and record into the file tree.
func file_tree_insert(name string, rec *FileRecord) {
	file_tree_insert_rec(&file_tree_root, name, rec)
}

// file_tree_insert_rec inserts a new node with the given name and record into the subtree rooted at the given root node.
...
// file_tree_store_rec dumps the subtree rooted at the given root node to the tree_store at iter.
// If flag is false, it dumps files; if true, it dumps directories.
...
// file_tree_store updates the tree_store widget with the current state of the file tree.
...
// file_tree_remove removes a node with the given name from the file tree.
// If merge_flag is true, it attempts to merge the deleted node with its parent.
...
// file_tree_remove_node removes a node from the file tree.
...
// file_tree_merge_parent_and_child merges a parent node with its only child.
