...
// Callback function for when paste operation is done
func paste_done_cb() {
    var be, en gtk.TextIter
    source_buf.GetStartIter(&be)
    source_buf.GetEndIter(&en)
...
// Callback function for opening a directory and processing all files within
...
// Dialog for selecting a file or directory
...
// Format the current file using gofmt
func gofmt_cb() {
    gofmt(cur_file)
}
