...
// file_opened checks if a file is already opened
func file_opened(name string) bool {
	_, found := file_map[name]
	return found
}

// delete_file_record removes a file record from the map and the file tree
...
// add_file_record adds a new file record to the map and the file tree
