...
// fnr_cb handles the callback for Find and Replace.
func fnr_cb() {
    fnr_dialog()
}

// fnr_dialog displays the Find and Replace dialog.
func fnr_dialog() {
    var fnr_cnt int = 0
...
// fnr_replace_all_local replaces all occurrences of the given string locally.
...
// fnr_replace_all_global replaces all occurrences of the given string globally.
...
// fnr_pre_cb updates the global flag and refreshes the scope.
...
// fnr_close_and_report closes the dialog and reports the number of replacements.
func fnr_close_and_report(dialog *gtk.Dialog, fnr_cnt int) {
    dialog.Destroy()
    bump_message(strconv.Itoa(fnr_cnt) + " replacements were done.")
}

// fnr_set_insert sets the insert position.
...
// fnr_refresh_scope refreshes the search scope.
...
// fnr_find_next finds the next occurrence of the given string.
...
// fnr_find_next_fill_global_map fills the global map with the locations of the string.
...
// fnr_replace replaces the given string with the replacement string.
...
// pop_string_from_map pops the first string from the given map.
...
// get_iter_at_mark_by_name sets the iterator to the given mark.
func get_iter_at