...
// fnr_cb function opens up the Find and Replace dialog box.
func fnr_cb() {
    fnr_dialog()
}

// fnr_dialog function is used to create the Find and Replace dialog box and define its functionalities.
func fnr_dialog() {
    var fnr_cnt int = 0
...
// fnr_replace_all_local function replaces all occurrences of a pattern with the replacement string within the current file only.
...
// fnr_replace_all_global function replaces all occurrences of a pattern with the replacement string across all files.
...
// fnr_pre_cb function is used to set the scope of the search and the state of the "insert_set" variable before performing a find and replace operation.
...
// fnr_close_and_report function is used to close the Find and Replace dialog box and report the number of replacements done.
func fnr_close_and_report(dialog *gtk.Dialog, fnr_cnt int) {
    dialog.Destroy()
    bump_message(strconv.Itoa(fnr_cnt) + " replacements were done.")
}

// fnr_set_insert function is used to set the cursor position before inserting a new text in the buffer.
...
// fnr_refresh_scope function refreshes the scope of the search based on whether the search is global or local.
...
// fnr_find_next function is used to find the next occurrence of a pattern.
...
// fnr_find_next_fill_global_map function fills the global map with files that match the search criteria.
...
// fnr_replace function replaces the current selection with the replacement string.
...
// pop_string_from_map function pops