...
// Calls fnr_dialog function
func fnr_cb() {
    fnr_dialog()
}

// Called by fnr_cb function to create a dialog window for Find and Replace
func fnr_dialog() {
    var fnr_cnt int = 0
    ...
    // Add entry and replacement fields to the dialog window
    ...
    // Connect functions to respective buttons
...
// Searches for and replaces text within the current buffer (without switching files)
...
// Searches for and replaces text within the entire project (switching files, if necessary)
...
// Sets the previous global flag and refreshes the scope
...
// Refreshes the scope of the search (global or local)
...
// Finds the next occurrence of the given pattern
...
// Fills the global map with file names that have the given pattern
...
// Replaces the entry with the given replacement and returns a tuple indicating number of replacements and whether
// another occurrence was found
...
// Pops a string element from the Map
