// Package main provides the entry point for the Tabby text editor.
package main
...
// refresh_title updates the title of the main window.
func refresh_title() {
	if "" == cur_file {
...
// buf_changed_cb is a callback function that is called when the content of the buffer changes.
func buf_changed_cb() {
	refresh_title()
}

// bump_message displays an information dialog with the given message.
...
// bump_question displays a warning dialog with the given message and returns true if the user clicked "Yes".
...
// init_tabby initializes the GUI of the Tabby text editor.
func init_tabby() {
	init_navigation()
	init_inotify()
	...
	new_item.AddAccelerator("activate",