...
func (v *SearchView) Init() {
    ...
    v.view.AppendColumn(gtk.NewTreeViewColumnWithAttributes("", gtk.NewCellRendererText(), "text", 0))
    v.view.SetHeadersVisible(false)
    v.view.Connect("cursor-changed", v.Select)
...
func (v *SearchView) PrevResult() {
    ...
    v.cursor = (v.cursor - 1 + v.size) % v.size
    v.SetCursor(v.cursor)
}
